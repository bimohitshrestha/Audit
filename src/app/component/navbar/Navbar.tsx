...
export default function Navbar() {
  ...
  return (
    <nav
      className={`transition-all duration-300 p-6 bg-gradient-to-r from-white via-blue-300 to-green-500 rounded-3xl m-8 text-kalo font-bold`}
    >
              ...
              {navigationItems.map(({ href, label }) => {
                ...
                return (
                  <Link key={href} href={href}>
                    <div
                      className={`relative px-4 py-2 rounded-md text-lg font-medium hover:text-level font-serif transition-all duration-200`}
            ...
            <div className="md:hidden relative">
              ...
              <AnimatePresence>
                {isMenuOpen && (
                  ...
                  >
                    <div className="shadow-lg px-4 pt-2 pb-4 bg-amber-100 rounded-lg">
                      {navigationItems.map(({ href, label }) => (
                        <Link key={href} href={href}>
                          <div
                            className={`block py-3 px-4 text-base rounded-md mb-1 transition-all hover:bg-amber-200 ${
                              activeItem === href
